...
// Setup the TCP / IP stack.
// This controller is a SERVER to the main frame client's commands
// *************************************************************************************************************
bool init_TCPIP_connections() {
	
		get_TCPIP_addresses();		// gets IP of this controllers client and server addresses
		get_Port_number();			// gets the port ID for this controller's client (to send reports to the server)
...
//START TCP SERVER
		client_connnected = alarmClient.Connect(alarmIp, Server_Port);		//start the client (reports)
...
void CheckClientConnection(){
	...
	if (!client_connnected && --wait_for_client_count <= 0){
		client_connnected = alarmClient.Connect(alarmIp, Server_Port);		
...
//											main();
//**********************************************************************************************************
//REMINDER: UPDATE VERSION NUMBER - now (Version: 01.09.04B)
//**********************************************************************************************************
...
//		access_SEEPROM(Version_EEPROM8, VERSION);
