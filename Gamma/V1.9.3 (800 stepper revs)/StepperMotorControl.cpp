...
void Lens_Motion_Handler() { // LENS RUNNING
		if (--LensSteps > 0){											// decrement number of pulses remaining and write the pulse to the port
			...
			else if (LensGoingHome || LensHomingToPos){					// the lense is in the safe or new absolute position forward?
...
void LensStepperHome(){
		if (Lens_homing_phase == 0){									// 
			...
			if (LensGoingHome){
				LensStepperToPosition(2 * LENS_SAFE_POS, FORWARD);			// run till the interrupt
			}
			else {
				LensStepperToPosition((Lens_new_position*2) + (2 * LENS_SAFE_POS), FORWARD);	// run till the interrupt
			}
		Lens_homing_phase++;
		LensGoingHome = false;
		return;
		...
		if (LensSteps <= 0){		//when the motor is in place, move to the next phase
			switch(Lens_restarting_phase){
				...
				case 2:
					...
					Lens_restarting_phase++;
					Lens_old_position = 0;
//					build_report (LensInit_pos, true, Lens_old_position, true);
//					send_report();
...
void CamStepperHome(){
		if (Cam_homing_phase == 0){									// 
			...
			if (CamGoingHome){
				CamStepperToPosition(2 * CAM_SAFE_POS, FORWARD);			// run till the interrupt
			}
			else {
				CamStepperToPosition((Cam_new_position*2) + (2 * CAM_SAFE_POS), FORWARD);	// run till the interrupt
			}
		Cam_homing_phase++;
		CamGoingHome = false;
		return;
		...
		if (CamSteps <= 0){		//when the motor is in place, move to the next phase
			switch(Cam_restarting_phase){
				...
				case 2:
					...
					Cam_restarting_phase++;
					Cam_old_position = 0;
//					build_report (CamInit_pos, true, Cam_old_position, true);
//					send_report();
					break;
			}
		}	
