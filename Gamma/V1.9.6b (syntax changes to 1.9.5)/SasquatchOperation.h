...
char VERSION[] = " (Version: 01.09.06b)]\r";
...
uint8_t *Client_Port_valid = (uint8_t *) SEEPROM_ADDR + 0x30;
uint8_t *Client_Port_EEPROM = (uint8_t *) SEEPROM_ADDR + 0x31;

uint8_t *Interval_valid = (uint8_t *) SEEPROM_ADDR + 0x38;
uint8_t *Heartbit_Interval_EEPROM = (uint8_t *) SEEPROM_ADDR + 0x39;
...
char CamNewPos[] = "[STATUS|Camera Stage stopped at position ";		// missing "[" is intentional
...
char LensNewPos[] = "[STATUS|Lens Stage stopped at position ";				// missing "[" is intentional
...
char no_valid_IP[] = "[WARNING|No valid EEPROM IP address found - displaying default addresses]\r";
...
char Command_rejected[] = "[WARNING|Command rejected - use MasterInitialize first]\r";
char Transport_not_ready[] = "[WARNING|Command rejected - Transport not initialized]\r";
...
char Server_check[] = "[DesktopServerCheck|";
char Command_OK[] = "[STATUS|Command Successful]\r";
char Par_error[] = "[ERROR|Parameter error]\r";
...
#define IDLE_LOOP_COUNT 300 // common check tasks every 300ms
#define HEARTBEAT_COUNT 10 * 3; // check hear bit every 5 * 3 Per sec * IDLE_LOOP_COUNT milisecs ~ 4.5 seconds for now
int heartbit_count = 0;
int heartbit_verify = 1;
