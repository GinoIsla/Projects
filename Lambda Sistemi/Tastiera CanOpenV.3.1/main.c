...
//	gi			17-01-2005	Version 1.2: Modify Configuration bits to correct writes to EEPROM
//	gi			14-07-2005	Version 2.0: Change sequence for reading keypad:
...
//	gi			09-09-2005	---------		Fixed several bugs, added Timer 1 to manage timings between
//											write of the LEDs,read of the keypad and error detection
//--------------------------------------------------------------------------------------------------------
//	gi			03-10-20006	Version 3.0		Version copmpiled for PIC18F4580 [MPLAB 7.40 and C18 v3.04]
//	gi			17-11-2006	Version 3.1		Bugs fixes for error reading EEPROM during init
/* ------------------------------------------- Include files: ------------------------------------------- */
...
M_KEY_ROW KEY_PAD_NEW[MAX_ROWS] ={0,0,0,0,0,0,0};  //recently read keypad
M_KEY_ROW KEY_PAD_PREV[MAX_ROWS] ={0,0,0,0,0,0,0}; //previous keypad states
M_KEY_ROW KEY_PAD_SENT[MAX_ROWS] ={0,0,0,0,0,0,0}; //last sent keypad states
unsigned char	DEBOUNCE_FLAG = _OFF;
...
//All rows checked, detect keypad changes
 ...
 if ((!Keychange) && (DEBOUNCE_FLAG == _ON))
	{
	if ((--debounce_count) > 0)
 ...
 mask.common_mask=_ON;
	...
	if (StatoUnita == CAN_STATE_OPERATIONAL)
	{
	 switch (Next_Group)
		{
		 case GROUP0:
			PORTEbits.RE1=_OFF;			//turn off other groups	
			PORTEbits.RE2=_OFF;
			LEDmask0 = LEDs[0];
			LEDmask1 = LEDs[1];
...
{
	PORTEbits.RE0=_OFF;
	PORTEbits.RE1=_OFF;			//turn off all columns
	PORTEbits.RE2=_OFF;			//
...
void CanIn(void)
{
 ...
 unsigned int Id_p;
  ...
  if (RXB_B [CAN_OPEN_PDO2].ST_B.IN)		//blinking
	...
	blink_rate = HALF_SECOND;					//restart the timer
    }           
}
