...
extern M_KEY_ROW KEY_PAD_NEW[MAX_ROWS];  //recently read keypad
extern M_KEY_ROW KEY_PAD_PREV[MAX_ROWS]; //previous keypad states
extern M_KEY_ROW KEY_PAD_SENT[MAX_ROWS]; //last sent keypad states
extern M_LED_mask mask;
M_LONGID Id_p;
...
const rom char y []= {3,2,3,0xE8,0,3,9,9,9,9};
...
//***************************************************//
void ReadEEprom (unsigned char EEaddress)
{
...
{
unsigned int Id_config,flash = 0;
...
unsigned char i,valid = FALSE;
 ...
 Id_config	= BOOTUP_MSG*0x20;				//4F8
 RXB_B [CAN_OPEN_IdNet].TXBSIDH = (Id_config  >> 8) & 0xFF;
 RXB_B [CAN_OPEN_IdNet].TXBSIDL = Id_config & 0xE0;
...
// Unused extended identifiers and filters
 ...
 INTCONbits.GIE = 1;		// Enable all interrupts
  ...
  Id_config = BOOT_REPLY;				//3F8
  Id_config  <<=5;
  TXB0SIDH 	= (Id_config  >> 8) & 0xFF;
  TXB0SIDL 	= Id_config & 0xE0;
  TXB0DLC	= 6;
  TXB0D0	= 0;
  TXB0D1	= 0;
  TXB0D2	= 0;
  ...
  do 
	{								//wait here until configuration is received
    _asm clrwdt _endasm
	PORTE  = 0x02;
    if (counter++ >= 80000)
     	{
...
init_cip ( void )   
{
 ...
 ADCON0 = 0x00;
 ADCON1 = 0x0F;		//all digital ports * change for PIC18F4580 (Ver. 3.0)
...
// TRISB = 0x8B;		//RB0, RB1 = KEYPAD, RB4..RB7 = ROWS KEYPAD
 ...
 TRISC = 0;			//LEDS OUT
 TRISD = 0;			//LEDs OUT
 TRISE = 0; 		//RE0, RE1, RE2 = LED & KEYPAD COMMON
 PORTA = 0;
 PORTB = 0;
 PORTC = 0;
 PORTD = 0;
 ...
 ReadEEprom(EEPROM_ID);
 Id = EEDATA;

 ReadEEprom(EEPROM_HEART_BEAT);
 heartbeat_rate.high_byte = EEDATA;
 heartbeat_count.high_byte = EEDATA;

 ReadEEprom(EEPROM_HEART_BEAT+1);
 heartbeat_rate.low_byte = EEDATA;
 heartbeat_count.low_byte = EEDATA;

 ReadEEprom(EEPROM_DEBOUNCE_RATE);
 debounce_period = (EEDATA+1)/2;	//debounce period given in 10ms, adjusted for 21ms cycle
...
*/
 ...
 ReadEEprom(EEPROM_CAN_BAUDRATE);
 baud_rate = EEDATA;
