...
PW_ON_PERIOD		EQU		.300*MILLI/LCD_TICK					;counts for a period of approx 300ms
PW_OFF_PERIOD		EQU		.3000*MILLI/LCD_TICK				;counts for a period of approx 3 seconds
DEBOUNCE_PERIOD		EQU		.200*MILLI/LCD_TICK					;debounce

	IF	DEBOUNCE_PERIOD < .150*MILLI/LCD_TICK
	ERROR:"DEBOUNCE_PERIOD OUT OF RANGE"
	ENDIF

AUTOLOCK_DEBOUNCE	EQU		.2000*MILLI/LCD_TICK				;2seconds

BLINK_PERIOD		EQU		.500*MILLI/LCD_TICK					;1/2 second blinks when adjusting the clock
IDLE_BTN_COUNT		EQU		.1000*MILLI/LCD_TICK				;button actions after 1 idle second

AUTOLOCK_ON_TIME	EQU		.1000*MILLI/(LCD_TICK*DEBOUNCE_PERIOD)	;the count of AUTOLOCK times is done with debouncing 
...
SEND_ON_PERIOD		EQU		.720						;in seconds, (+ a random number) for repeat ON msg
MSG_PAUSE_PERIOD	EQU		.32*MILLI/LCD_TICK			;30 ms betwwen repeats of the same message

HEAT_MSG_COUNTDWN	EQU		.300*MILLI/LCD_TICK			;delay before sending the last heat message

BATT_HIGH_THRESHLD	EQU		(.60*.255)/.100				;substitute the percent load (.65) by appropriate number
BATT_LOW_THRESHLD	EQU		(.53*.255)/.100				;substitute the percent load (.40) by appropriate number
