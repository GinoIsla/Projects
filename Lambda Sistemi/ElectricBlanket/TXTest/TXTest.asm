;----------------------------------------------------------------
;TEST Tranmitter / Remote control for an electric blanket.  
;
;   TTTTTTTTTT	EEEEEEEEE	SSSSSSSSSSS	TTTTTTTTTTTTT
;	TT	EE		SS	 SS	      TT
;	TT	EE		SS		      TT
;	TT	EEEEEE		SSSSSSSSSSS	      TT
;	TT	EE			 SS	      TT
;	TT	EE		SS	 SS	      TT
;	TT	EEEEEEEEE	SSSSSSSSSSS	      TT
;
;Funtions: Simulute different transmissions modes dependign on the MODE button position
;
; Programmer:	Gino Isla for Lambda Sistemi, Milano
; Created:		03-12-2005	Version 1.0
; Clock:		4Mhz
; IDE: 			MPLAB v7.20
;----------------------------------------------------------------
...
main						;
	...
	movlw	AUTOLOCK_DEBOUNCE
	movlw	BATT_HIGH_THRESHLD
	movlw	BATT_LOW_THRESHLD
	movlw	MSG_PAUSE_PERIOD
	movlw	LCD_PRESCALE
	movlw	LCD_FRAME_FREQ
	movlw	PW_ON_PERIOD
	movlw	PW_OFF_PERIOD
...
;interrupts and ports				;
	...
	MOVWF	TRISE				;
						;
	MOVLW	b'10000000'			;no pullups, tmr0 internal, prescale assigned to tmr0,<2:0>=PS tmr0 = 1:2 ***FOR NOW
...
_STAY_UP					;
	...
	CALL	SEND_MESSAGES			; so go do other jobs
						;
	NOP					;
	NOP					;
	NOP					;
	...
	CALL	HANDLE_BUTTONS			;
						;
	BTFSC	BATTERY_LOW			;block all actions of the batteries are low
	GOTO	_SNOOZE				;
...
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;-----------------------------------------------;---------------- THIS PROJECT'S TEST MODIFICATIONS: ---------
SEND_MESSAGES					;
	...
	BTFSC	THREEHR				;
	GOTO	_THREE_HOURMSG			;
	GOTO	_TWELVE_HOURMSG			;12hr or OFF
						;
_ONE_HOURMSG					;
	DECFSZ	PAUSE_COUNT,F			;
	GOTO	_MSG_FIN			;in this mode, respect the pause between messages but continue
						; to send them ad infinitum
	BSF	SND_ON_MSG			;
	MOVF	REXMIT_COUNT,F			;-used as a flag-
	BTFSC	STATUS,Z			;
	GOTO	_ON_MSG				;	
	GOTO	_RE_SEND			;
						;
_THREE_HOURMSG					;
	BCF	TX_ENABLE			;
	BANK1					;In this mode just enable (active low) TX_ENABLE
	BCF	TXENBL_TRIS			;
	BANK0					;
	BCF	TX_ENABLE			;
	BSF	PWM_HCS				;hold the RF-TX pin always high
	GOTO	_MSG_FIN			;
						;
_TWELVE_HOURMSG					;
	CLRF	REXMIT_COUNT			;
	...
	BSF	TXENBL_TRIS			;
	BANK0					;
	BCF	PWM_HCS				;hold the RF-TX pin low when not in transmission
	GOTO	_MSG_FIN			;
;-----------------------------------------------;---------------- THIS PROJECT'S TEST MODIFICATIONS: ---------
						;
_NEW_MSG					; else
...
_LOOP_DELAY					;
	DECFSZ	TEMP_BYT0_B0,F			;
	GOTO	_LOOP_DELAY			;
	BSF	_GIE				;enable global ints	for
						;
	MOVLW	MSG_PAUSE_PERIOD		;
	MOVWF	PAUSE_COUNT			;
	CALL	INIZIO_TX_HCS			;
...
;======================================= NEW ACTION FOR AUTO-OFF TIMER =====
	...
	BTFSC	TWELVEHR			;
	GOTO	_MODE_OFF			;
	GOTO	_INVALID_BUTTON			;resets flags, resets relock count and goes out
						;
_MODE1HR					;
...
_NEW_TEMP_VARS					;
	...
	GOTO	_INVALID_BUTTON			;REMOVE THESE 2 LINES IF NOT NEEDED
						;
	BTFSC	AUTOLOCK_ON			;
	GOTO	_INVALID_BUTTON			;Temperature variations accepted only if unlocked
						;
...
_DISPLAY_ADJUSTMENTS				;
	BSF	LCD_UPD				;
	CALL	UPDATE_LCD			;
	GOTO	_RESET_RELOCK			;DONE ADJUSTING TIME
						;
_INVALID_BUTTON					;
	BCF	BTN_ACT_PDG			;
	CLRF	NEW_BUTTON_CONFIG		;
...
FILL_RESEND_PERIOD				;
	...
	INCF	TEMP_BYT1_B0,W			;this INCF is to make the countdown algorithm work fotr the last FF period
...
_RELOAD_AO_MINUTES				;
	...
	GOTO	PREHEAT_COUNTDOWN		;
_AO_DONE					;
	CLRF	AUTO_OFF_HOURS			;AUTO SHUT-OFF count reached, 
	BCF	DEVON				;Change the display to "OFF"
	BCF	TEMPETR				;
	BSF	DEVOFF				;Send and OFF message to the base
	BCF	ONEHR				;
	BCF	THREEHR				;
	BCF	TWELVEHR			;
	BSF	SND_OFF_MSG			;(no beep)
	MOVLW	SEND_OFF_PERIOD			;
	MOVWF	RESEND_OFF_COUNT		;load counter to repeat the off message after 15'
	MOVF	TMPTR_VALUE,W			;
