...
;Updates:
;----------------------------------------------------------------
;GI 			22-12-05	Add ":" blinking every second (new requirement)
;				 	Add new debounce timer for temperature changes so only the last one beeps
;----------------------------------------------------------------
;GI			04-01-06 	Add random seconds resend of SND_BOO_OFF_MSG
;----------------------------------------------------------------
;GI			10-01-06 	Random seconds resends optional, added #DEFINES
;----------------------------------------------------------------
;JC			20-01-06	reminder to remove initial zero on visualized clock to be quoted and built for
;					the next release fo this software
;----------------------------------------------------------------
...
BOOST_SECONDS					;seconds counter PRE-HEAT seconds
...
TMR_1SECOND					;one second counter to be used TMR2
;=====================================================
...
;	ERROR	"MISSING MODIFY: INITAL ZERO ON CLOCK"
...
;===================================== NEW BOOST ACTION ==============
	...
	MOVWF	TMPTR_VALUE			;
	MOVLW	BOOST_PERIOD			;
	MOVWF	BOOST_MINUTES			;start BOOST timers
	MOVF	RT_SECONDS,W			;
	MOVWF	BOOST_SECONDS			;
	BCF	F_SEND_BOOFF			;
	BCF	F_RESEND_BOOFF			;
...
_HANDLE_FIN					;
	RETURN					;
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;Fill the RESEND_ON_COUNT with either a pseudo random number with a spread that 
;goes from 12' to 15' or a fixed counter
...
#ELSE						;
	MOVLW	HIGH (SEND_ON_PERIOD)+1		;make the countdown algorithm work for the last FF period
	MOVWF	RESEND_ON_COUNT_H		;
	MOVLW	LOW (SEND_ON_PERIOD)		;
	MOVWF	RESEND_ON_COUNT_L		;
#ENDIF						;
...
_BATT_FIN					;
	RETURN					;
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;seconds actions, time base LCD period
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
SECONDS_TASKS					;
	DECFSZ	TMR_1SECOND,F			;
	GOTO	_SECONDS_END			;
						;
	BTFSC	CLCKSET_ACT			;blink : if we're NOT adjusting the clock
	GOTO	_BOOST_OFF_SECS			;
						;
...
;...............................................;
_BOOST_OFF_SECS					;
	MOVLW	b'11000000'			;are we treating the BOOST_OFF_MSG seconds counter?
	ANDWF	LCD_CHAR5,W			;
	BTFSC	STATUS,Z			;
	GOTO	_RESEND_ON_MSG			;
						;
	DECFSZ	BOOST_SECONDS,F			;
	GOTO	_SECONDS_END			;this jump avoids a useless refresh during this minute
						;
	BCF	BOOST				;now send BOOST-OFF and setupf the counter to 
	BCF	BOOST_ACT			; resend 
	MOVF	SAVED_TEMPERATURE,W		;
	MOVWF	TMPTR_VALUE			;
	BCF	BEEPER				;
	BSF	SND_BOO_OFF_MSG			;turn pre heat off in the base
						;
	BTFSS	F_SEND_BOOFF			;first send?
	GOTO	_BOOST_CYCLE_END		;
						;
#IFDEF RANDMOM_BOOST_OFF			;
	MOVLW	0x1F				;prepare resend of boost
	ANDWF	TMR0,W				;random wait of 1 to 32 seconds
	MOVWF	BOOST_SECONDS			;
	INCF	BOOST_SECONDS,F			;
#ELSE						;
	MOVLW	RESEND_BOOST_PERIOD		;fixed restransmit
	MOVWF	BOOST_SECONDS			;
#ENDIF						;
	BCF	F_SEND_BOOFF			;
	BSF	F_RESEND_BOOFF			;
	GOTO	_SECONDS_END			;avoids a useless refresh
_BOOST_CYCLE_END				;
	BCF	F_RESEND_BOOFF			;
	GOTO	_SECONDS_END			;avoids a useless refresh
;...............................................;
_RESEND_ON_MSG					;this is the REFRESH message
	BTFSS	DEVON				;
	GOTO	_SECONDS_END			;
...
_RT_SECONDS					;
	...
	INCF	RT_SECONDS,F			;
;...............................................;
;HANDLE OTHER REAL TIME SECONDS COUNTERS
;...............................................;
...
PREHEAT_COUNTDOWN				;
	...
	GOTO	_RT_HOURS			;
	BSF	F_SEND_BOOFF			;modification: instead of sending the 
						;BOO_OFF_MSG, set the flag for the seconds countdown
						;
;============================= HOURS COUNTERS ===============================
_RT_HOURS					;
