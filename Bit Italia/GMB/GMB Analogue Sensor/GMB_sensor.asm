;************************************************************
;	TITLE:	ECHO BUS multinodal control ----- Analogue Sensor Version (2 ports IN)
;	Programmer: GINO ISLA for of BIT Srl.
;	REVISION HISTORY:	14-October-2004
;	LAST UPDATE:		
;	FREQUENCY: 8MHz
;************************************************************
			list	p=16f688
...
#include	<p16f688.inc>
#include 	<GMB_sensor.h>
#include	<c:\embedded\macros.inc>

	__CONFIG	_CP_OFF&_WDT_ON&_BOD_OFF&_PWRTE_ON&_HS_OSC&_PWRTE_ON&_CPD_OFF&_MCLRE_OFF
	ERRORLEVEL -302
...
;			GOTO	_NO_RESET			;	
			...
			CLRF	PORTC				;
			CLRF	IOCA				;
			BSF		IOCA,2				;Echo Bus RX = RA2
;A/D CONVERTER							;
;=============							;
			MOVLW	.7					;
			MOVWF	CMCON0				;	
			MOVLW	b'10000001'			;left justtified,vref,A/D powered on
			MOVWF	ADCON0				;
			BSF		STATUS,RP0			;
			MOVLW	b'00110000'			;RC0=analogue chan 1 = AN4, RC1=analogue chan 2 = AN5
			MOVWF	ANSEL				;
			MOVLW	b'00000100'			;fosc/64
			MOVWF	ADCON1				;
			BCF		STATUS,RP0			;
										;
			MOVLW	0XFF				;
			MOVWF	TRISA				;RA2=EchoBUS RX, RC4=EchoBUS TX
			MOVWF	TRISC				;RC0=AN0, RC1=AN1
			CLRF	PIE1				;all peripheral are polled
			MOVLW	b'10001011'			;OPTION<6>=0=int on falling edge RA0, ...SELECT TMR0 AND WDT OPTION
...
_DELAY_INI	NOP							;
			...
			MOVLW	EE_SAVED_SAMPLE		;input references
			CALL	READ_EEPROM			;
...
;			BANKSEL	TIMER_0_UP			;
			...
			MOVWF	T1CON				;
			MOVLW	b'10010000'			;
			MOVWF	INTCON				;INTCON<4>=enbable RA0 ints (Echo Bus RX)
										;
_MAIN_LOOP	CLRWDT						;
...
GET_SAMPLES								;
			...
			BTFSS	STATUS,Z			;
			GOTO	$+3					;
			MOVLW	b'00010000'			;READ PORT 0 (AN4)
			GOTO	$+2					;
			MOVLW	b'00010100'			;READ PORT 1 (AN5)
										;
...
_NXT_SUM	INCF	FSR,F				;
	   ...
	   M_CMPL_JL	SAMPLE_CNTR,4,_NXT_SUM
			...
			RETURN						;
;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;Get states of inputs and report back to the CPU.
...
SND_PORT_PARS							;
		M_CMPL_JNE	MSG_SUBT,1,_PORTS1	; decide if its port pair 0 or 1
			MOVF	PERIOD_AVG_0,W		;
			MOVWF	MSG_DISC			;
			CLRF	MSG_ARGT			;instead of outport (only sensor version)
			GOTO	_SND_PP				;
										;
...
;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;Send a message with the REFERENCE_SAMPLE.  The message is treated more or less as if it 
;was an alarm.
...
;---------------------------------------;Testing sub-type from here on
			...
			BTFSC	STATUS,Z			;
			GOTO	_REPLY_TO_MY_MSG	;reply to a message sent by me
										;
			MOVF	MSG_SUBT,W			;
...
_MSG_DONE	BSF		BUS_OUT				;release bus
			...
			BSF		STATUS,RP0			;
			BSF		TRISC,4				;bus out in high impedance (input)
			BCF		STATUS,RP0			;
